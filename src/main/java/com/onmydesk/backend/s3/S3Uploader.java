package com.onmydesk.backend.s3;
...
public class S3Uploader {
    ...
    // MultipartFile을 전달받아 리사이즈된 이미지 파일로 전환한 후 S3에 업로드
    public String upload(MultipartFile multipartFile, String dirName) throws IOException {

        File uploadFile = convert(multipartFile)
                .orElseThrow(() -> new IllegalArgumentException("MultipartFile -> File 전환 실패"));
        File resizedFile = resizeImage(uploadFile); // 이미지 리사이징
        String uploadImageUrl = upload(resizedFile, dirName); // 리사이즈된 파일 업로드

        removeNewFile(uploadFile);  // 원본 파일 삭제
        removeNewFile(resizedFile); // 리사이즈 파일 삭제
